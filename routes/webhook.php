...
if(@str_contains(env('APP_URL'),'aipen.test')) Route::any('webhook/paypal-ipn-action',[WebhookPayment::class,'paypal_ipn_action'])->name('paypal-ipn-action');
else Route::post('webhook/paypal-ipn-action',[WebhookPayment::class,'paypal_ipn_action'])->name('paypal-ipn-action');

Route::any('webhook/stripe-ipn/{buyer_user_id}/{parent_user_id}/{package_id}',[WebhookPayment::class,'stripe_ipn'])->name('stripe-ipn');
