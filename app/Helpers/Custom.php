...
if ( ! function_exists('set_agency_config')){
    function set_agency_config()
    {
        ...
        $thirdpary_api_settings = $admin_user_data->thirdpary_api_settings ?? '';
        ...
        $thirdpary_api_settings =  isset($thirdpary_api_settings) && !empty($thirdpary_api_settings) ? json_decode($thirdpary_api_settings) : [];
        if(Auth::user()){
            $default = $thirdpary_api_settings->ai->default ?? '';
            set_thirdparty_api_config($default);
        }

        return ['app_name'=>$app_name,'app_logo'=>$app_logo,'app_logo_alt'=>$app_logo_alt,'app_favicon'=>$app_favicon,'email_settings'=>$email_settings,'thirdpary_api_settings'=>$thirdpary_api_settings,'force_email_verify'=>$force_email_verify];
    }
}
...
if ( ! function_exists('set_thirdparty_api_config')) {
    function set_thirdparty_api_config($default = '',$user_id=null){
        if(empty($user_id)) $user_id = 1;
        //no default element passed, find the default
         if(empty($default) || $default==0){
            $settings_data = DB::table('settings')->where(['user_id'=>$user_id])->select('thirdpary_api_settings')->first();
            $thirdpary_api_settings =  isset($settings_data->thirdpary_api_settings) && !empty($settings_data->thirdpary_api_settings) ? json_decode($settings_data->thirdpary_api_settings) : [];
            $default = $thirdpary_api_settings->ai->default ?? '';
        }

        if(!empty($default) && $default!=0){
            if(is_numeric($default)) $thirdpary_api_settings_data = DB::table('settings_thirdparty_apis')->where(['id'=>$default])->first();
            else if($default=='#RANDOM#') $thirdpary_api_settings_data = DB::table('settings_thirdparty_apis')->where(['api_type'=>'ai','user_id'=>$user_id])->inRandomOrder()->first();

            if(!empty($thirdpary_api_settings_data)){
                $thirdpary_api_settings_data_decoded = json_decode($thirdpary_api_settings_data->settings_data,true);
                $system_prompt = $thirdpary_api_settings_data->system_prompt ?? '';
                $api_key = $thirdpary_api_settings_data_decoded['api-key'] ?? '';
                config(['api.openai-key' => $api_key]);
                config(['api.openai-system-prompt' => $system_prompt]);
                return true;
            }
            return false;
        }
...
if ( ! function_exists('convert_datetime_to_timezone'))
{
  function convert_datetime_to_timezone($input_datetime='',$to_timezone='',$display_timezone=false,$date_format='',$from_timezone='UTC')
  {
      ...
      if(empty($to_timezone)) $to_timezone = 'UTC';
...
if ( ! function_exists('convert_duration_to_minutes'))
{
    function convert_duration_to_minutes($duration='',$round_up=true){
        if(empty($duration)) return 0;
        $parts = explode(':',$duration);
        $duration = 0;
        if(count($parts)==3){
            $duration += $parts[0]*60 + $parts[1];
            if($parts[2]>0) $duration += $parts[2]/60;
        }
        if(count($parts)==2){
            $duration += $parts[0];
            if($parts[1]>0) $duration += $parts[1]/60;
        }
        else {
            if($parts[0]>0) $duration += $parts[0]/60;
        }
        return $round_up ? ceil($duration) : $duration;
    }
}
...
if ( ! function_exists('format_price'))
{
    ...
    function format_price($price=0,$format_settings=null,$discount_settings=null,$return_settings=['which_price'=>'both','display_currency'=>true,'return_raw_array'=>false])
    {
        ...
        if($sale_price<$price){
            $display_price = '<del class="fs-70p">'.$price.'</del> '.$sale_price_formatted;
        }
  ...
  function array_format_datatable_data($result=array(), $columns=array(), $start=0,$primary_key='id')
    {
        ...
        foreach ($result as $key => $single_row) {
            ...
            if($have_checkbox)
            {
              ...
              $str = '<div class="form-check form-switch d-flex justify-content-center pt-2"><input class="form-check-input datatableCheckboxRow" name="datatableCheckboxRow[]" id="datatableCheckboxRow'.$primary_val.'" type="checkbox"  value="'.$primary_val.'"></div>';
              $temp[1] = $str;
            }
...
if( ! function_exists('get_openai_temperature_list')) {
    function get_openai_temperature_list()
    {
        return [
            "0"=>__("Predictable"),
            "0.1"=>__("Slightly Creative"),
            "0.2"=>__("Somewhat Creative"),
            "0.3"=>__("Moderately Varied"),
            "0.4"=>__("Moderately Creative"),
            "0.5"=>__("Moderately Diverse"),
            "0.6"=>__("Highly Varied"),
            "0.7"=>__("Highly Creative"),
            "0.8"=>__("Highly Unconventional"),
            "0.9"=>__("Extremely Divergent"),
            "1"=>__("Default"),
            "1.1"=>__("Highly Random"),
            "1.2"=>__("Highly Chaotic"),
            "1.3"=>__("Extremely Chaotic"),
            "1.4"=>__("Extremely Chaotic and Diverse"),
            "1.5"=>__("Chaotic and Unpredictable"),
            "1.6"=>__("Extremely Unpredictable and Chaotic"),
            "1.7"=>__("Nearly Incoherent"),
            "1.8"=>__("Incoherent and Meaningless"),
            "1.9"=>__("Highly Incoherent and Illogical"),
            "2"=>__("Utterly Chaotic and Meaningless")
        ];
    }
}

//presence_penalty values
if( ! function_exists('get_openai_input_repeatness_list')) {
    function get_openai_input_repeatness_list(){
        return [
            '-2'=>__('Maximum Absence'),
            '-1.9'=>__('Highly Absent'),
            '-1.8'=>__('Strongly Absent'),
            '-1.7'=>__('Severely Absent'),
            '-1.6'=>__('Extremely Absent'),
            '-1.5'=>__('Very Absent'),
            '-1.4'=>__('Quite Absent'),
            '-1.3'=>__('Moderately Absent'),
            '-1.2'=>__('Reasonably Absent'),
            '-1.1'=>__('Fairly Absent'),
            '-1'=>__('Moderately Prohibitive'),
            '-0.9'=>__('Reasonably Prohibitive'),
            '-0.8'=>__('Quite Prohibitive'),
            '-0.7'=>__('Very Prohibitive'),
            '-0.6'=>__('Highly Prohibitive'),
            '-0.5'=>__('Extremely Prohibitive'),
            '-0.4'=>__('Quite Restrictive'),
            '-0.3'=>__('Moderately Restrictive'),
            '-0.2'=>__('Somewhat Restrictive'),
            '-0.1'=>__('Slightly Restrictive'),
            '0'=>__('Default'),
            '0.1'=>__('Slightly Encouraging'),
            '0.2'=>__('Somewhat Encouraging'),
            '0.3'=>__('Moderately Encouraging'),
            '0.4'=>__('Fairly Encouraging'),
            '0.5'=>__('Reasonably Encouraging'),
            '0.6'=>__('Quite Encouraging'),
            '0.7'=>__('Very Encouraging'),
            '0.8'=>__('Highly Encouraging'),
            '0.9'=>__('Extremely Encouraging'),
            '1'=>__('Maximum Encouragement'),
            '1.1'=>__('Slightly Discouraging'),
            '1.2'=>__('Somewhat Discouraging'),
            '1.3'=>__('Moderately Discouraging'),
            '1.4'=>__('Fairly Discouraging'),
            '1.5'=>__('Reasonably Discouraging'),
            '1.6'=>__('Quite Discouraging'),
            '1.7'=>__('Very Discouraging'),
            '1.8'=>__('Highly Discouraging'),
            '1.9'=>__('Extremely Discouraging'),
            '2'=>__('Maximum Discouragement')
        ];
    }
}

//frequency_penalty values
if( ! function_exists('get_openai_uniqueness_list')) {
    function get_openai_uniqueness_list()
    {
        return [
            '-2'=>__('Maximum Repeat'),
            '-1.9'=>__('Highly Encouraged'),
            '-1.8'=>__('Encouraged'),
            '-1.7'=>__('Boosted'),
            '-1.6'=>__('Stimulated'),
            '-1.5'=>__('Enhanced'),
            '-1.4'=>__('Assisted'),
            '-1.3'=>__('Supported'),
            '-1.2'=>__('Moderately Encouraged'),
            '-1.1'=>__('Somewhat Encouraged'),
            '-1'=>__('Slightly Encouraged'),
            '-0.9'=>__('Softly Encouraged'),
            '-0.8'=>__('Mildly Encouraged'),
            '-0.7'=>__('Tolerated'),
            '-0.6'=>__('Permitted'),
            '-0.5'=>__('Allowed'),
            '-0.4'=>__('Accepted'),
            '-0.3'=>__('Neutral'),
            '-0.2'=>__('Controlled'),
            '-0.1'=>__('Mildly Penalized'),
            '0'=>__('Default'),
            '0.1'=>__('Slightly Penalized'),
            '0.2'=>__('Moderately Penalized'),
            '0.3'=>__('Strongly Penalized'),
            '0.4'=>__('Severely Penalized'),
            '0.5'=>__('Highly Penalized'),
            '0.6'=>__('Heavily Penalized'),
            '0.7'=>__('Harshly Penalized'),
            '0.8'=>__('Strictly Penalized'),
            '0.9'=>__('Stringently Penalized'),
            '1'=>__('Maximum Penalty'),
            '1.1'=>__('Intensified'),
            '1.2'=>__('Stringent'),
            '1.3'=>__('Rigorous'),
            '1.4'=>__('Strict'),
            '1.5'=>__('Absolute'),
            '1.6'=>__('Prohibitive'),
            '1.7'=>__('Severely Prohibitive'),
            '1.8'=>__('Strongly Prohibitive'),
            '1.9'=>__('Highly Prohibitive'),
            '2.0'=>__('Maximum Prohibition')
        ];
    }
}

if( ! function_exists('get_openai_endpoint_list')) {
    function get_openai_endpoint_list()
    {
        return [
            'Text'=>[
                'completions'=>[
                    'title'=>__('Text Completion'),
                    'models'=>[
                    ...
                    'output'=>['textarea'=>__('Textarea'),'code'=>__('Code Editor')],
                    'defaultParams'=>[],
                    'icon'=>'mdi mdi-text-box-check text-primary'
                ],
                // 'edits'=>[
                //     'title'=>__('Text Edit'),
                //     'models'=>['text-davinci-edit-001'=>'text-davinci-edit-001'],
                //     'output'=>['textarea'=>__('Textarea')],
                //     'defaultParams'=>[],
                //     'icon'=>'mdi mdi-text-box-edit text-primary'
                // ]
            ],
            // 'Code'=>[
            //     'completions'=>[
            //         'title'=>__('Code Completion'),
            //         'models'=>['code-davinci-002'=>'code-davinci-002','code-cushman-001'=>'code-cushman-001'],
            //         'output'=>['code'=>__('Code Editor')],
            //         'defaultParams'=>[],
            //         'icon'=>'mdi mdi-code-array text-info'
            //     ],
            //     'edits'=>[
            //         'title'=>__('Code Edit'),
            //         'models'=>['code-davinci-edit-001'=>'code-davinci-edit-001'],
            //         'output'=>['code'=>__('Code Editor')],
            //         'defaultParams'=>[],
            //         'icon'=>'mdi mdi-code-braces-box text-info'
            //     ]
            // ],
            'Chat'=>[
                'chat/completions'=>[
                    'title'=>__('Chat Completion'),
                    'models'=>[
                    ...
                    'output'=>['textarea'=>__('Textarea'),'code'=>__('Code Editor')],
                    'defaultParams'=>[],
                    'icon'=>'mdi mdi-message text-success'
                ]
            ],
            'Image'=>[
                'images/generations'=>[
                    ...
                    'models'=>[],
                    'output'=>['display'=>__('Display')],
                    'defaultParams'=>['image'],
                    'icon'=>'mdi mdi-image-plus text-warning'
                ],
                'images/variations'=>[
                    'title'=>__('Image Variation'),
                    'models'=>[],
                    'output'=>['display'=>__('Display')],
                    'defaultParams'=>['image'],
                    'icon'=>'mdi mdi-image-refresh text-warning'
                ]
            ],
            'Audio'=>[
                'audio/transcriptions'=>[
                    'title'=>__('Text to Speech Transcription'),
                    'models'=>['whisper-1'=>'whisper-1'],
                    'output'=>['textarea'=>__('Textarea')],
                    'defaultParams'=>['textarea'=>__('Textarea')],
                    'icon'=>'mdi mdi-book-music text-danger'
                ],
                'audio/translations'=>[
                    'title'=>__('Text to Speech Translation'),
                    'models'=>['whisper-1'=>'whisper-1'],
                    'output'=>['textarea'=>__('Textarea')],
                    'defaultParams'=>['textarea'=>__('Textarea')],
                    'icon'=>'mdi mdi-google-translate text-danger'
                ],
            ]
        ];
    }
...
if ( ! function_exists('get_public_path'))
{
    function get_public_path($param='') {
        $replace = get_domain_only(ENV('APP_URL'))=='aipen.test' ? 'public' : 'html';
...
//USED BY MULTILINGUAL EDITOR PACKAGE
