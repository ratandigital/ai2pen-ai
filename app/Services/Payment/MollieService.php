...
class MollieService implements MollieServiceInterface{
    ...
    function set_button()
    {
        ...
        $receipt = "" . time() . "";
        $mollie_lang = !empty($this->button_lang) ? $this->button_lang : __("Mollie Payment");
        ...
        session(['order_id_session'=>$order_id]);
        ...
        if ($this->secondary_button) {
			...
			<a href='{$checkout_link}' class='list-group-item list-group-item-action flex-column align-items-start' id='mollie-payment-button'>
			    <div class='d-flex w-100 align-items-center'>
			      <small class='text-muted'><img class='rounded' width='60' height='60' src='" . asset('assets/images/mollie.png') . "'></small>
			      <h5 class='mb-1'>" . $mollie_lang . "</h5>
			    </div>
			</a>";
    ...
    {
        $order_id_session = "order_id_" . $this->ec_order_id;
        $transaction_id = session('order_id_session');
        $response = array();
