...
class Member extends Home
{
    public function __construct(MollieServiceInterface $mollie_service, PaystackServiceInterface $paystack_service, RazorpayServiceInterface $razorpay_service,MyfatoorahServiceInterface $myfatoorah_sevice, MercadopagoServiceInterface $mercadopago_service, InstamojoServiceInterface $instamojo_service,PaymayaServiceInterface $paymaya_service,ToyyibpayServiceInterface $toyyibpay_service,XenditServiceInterface $xendit_service,SenangpayServiceInterface $senangpay_service,PaypalServiceInterface $paypal_service,StripeServiceInterface $stripe_service,YoomoneyServiceInterface $yoomoney_service,FlutterwaveServiceInterface $flutterwave_service)
    {
        ...
        else $this->set_global_userdata(false);
    ...
    public function account_action(Request $request)
    {
        ...
        [
            ...
            'profile_pic'=>'nullable|sometimes|image|mimes:png,jpg,jpeg,webp'
        ];
    ...
    {
        if($this->is_admin || $this->is_manager) abort('403');
        ...
        $data["payment_package"] = $this->get_packages_parent();
   ...
   public function buy_package($id=0)
   {
       ...
       if($paypal_status=='1')
       {
           ...
           $this->paypal->mode = $paypal_mode;
           $this->paypal->cancel_url =route('paypal-subscription-action',['buyer_user_id'=>$this->user_id,'parent_user_id'=>$this->parent_user_id,'package_id'=>$id]);
           ...
           $notify_url = get_domain_only(env('APP_URL'))=='aipen.test' ? 'https://ezsoci.com/botsailor-test-ipn/paypal.php' : route('paypal-ipn',$paypal_mode);
           $this->paypal->paypal_client_id = $paypal_client_id;
           $this->paypal->paypal_client_secret = $paypal_client_secret;
           $this->paypal->paypal_app_id = $paypal_app_id;
       ...
       if($razorpay_status=='1' && !empty($razorpay_status)){
           ...
           $store_favicon = asset("assets/images/logo.png");
       ...
       if($paystack_status=='1' && !empty($paystack_status)){
           ...
           $this->paystack_class_ecommerce->img_url=asset("assets/images/logo.png");
       ...
       if($mercadopago_status=='1'){
           ...
           $marcadopago_country = $mercadopago_data->mercadopago_country;
       ...
       if($mollie_status=='1' && !empty($mollie_status)){
           ...
           $this->mollie_class_ecommerce->img_url=asset("assets/images/logo.png");
    ...
    public function transaction_log()
    {
        ...
        if($this->is_admin) {
            ...
            $currency = isset($payment_config->currency) ? $payment_config->currency : "USD";
            $year = date("Y");
            $lastyear = $year - 1;
            $month = date("m");
            $date = date("Y-m-d");
    ...
    public function transaction_log_data(Request $request)
    {
        ...
        if($this->is_admin) array_push($display_columns,'invoice_url');
    ...
    public function list_payment_api_log_data(Request $request)
    {
        ...
        $search_value = !is_null($request->input('search.value')) ? $request->input('search.value') : '';
        $date_range = !is_null($request->input('date_range')) ? $request->input('date_range') : '';
        $display_columns = array("#","CHECKBOX",'id','buyer_email','buyer_username','payment_method', 'call_time', 'request_data', 'error',);
        $search_columns = array('buyer_users.email', 'buyer_username','payment_method');
        ...
        $select= ["payment_api_logs.*","buyer_users.name as buyer_username","buyer_users.email as buyer_email"];
        $query = DB::table($table)->select($select);
        ...
        foreach ($info as $key => $value)
        {
            ...
            $value->request_data = "<textarea class='hide-scroll form-control form-control-lg'>".$value->api_response."</textarea>";
        }
    ...
    {
        if(!$this->is_admin || !has_module_access($this->module_id_settings, $this->module_ids, $this->is_admin)) abort('403');
        $data['body'] = 'member/settings/payment-settings';

        $data['xdata'] = DB::table('settings_payments')->where(['user_id'=>$this->user_id])->first();
        $data['iframe'] = false;
    ...
    public function payment_settings_action(Request $request)
    {
        ...
        return redirect(route('payment-settings'));
    }
    ...
    public function general_settings_action(Request $request)
    {
        ...
        [
            ...
            'default_ai_api' => 'nullable|sometimes',
        ...
        $rules['ai_chat_icon'] = 'nullable|sometimes|image|mimes:png,jpg,jpeg,webp|max:100';
        ...
        if ($this->is_admin) {
            ...
            if ($request->file('ai_chat_icon')) {

                $file = $request->file('ai_chat_icon');
                $extension = $request->file('ai_chat_icon')->getClientOriginalExtension();
                $filename =  $this->is_admin ? 'ai_chat_icon.' . $extension : $this->user_id . '-ai_chat_icon.' . $extension;
                $upload_dir_subpath = 'public/agency';

                if(env('AWS_UPLOAD_ENABLED')){
                   try {
                       $upload2S3 = Storage::disk('s3')->putFileAs('agency', $file,$filename);
                       $insert_data['ai_chat_icon'] = Storage::disk('s3')->url($upload2S3);
                   }
                   catch (\Exception $e){
                       $error_message = $e->getMessage();
                   }
                    ...
                    $request->file('ai_chat_icon')->storeAs(
                        $upload_dir_subpath, $filename
                    );
                    $insert_data['ai_chat_icon'] = asset('storage/agency').'/'.$filename;
                }
            }
        ...
        if(!$this->is_member){
            ...
            $force_email_verify = request()->force_email_verify ?? '0';
            $insert_data['force_email_verify'] = $force_email_verify;
        }
        ...
        $thirdpary_api_settings = [
            'ai' => ['default'=>$validate_data['default_ai_api'] ?? null]
        ];
        $insert_data['thirdpary_api_settings'] = json_encode($thirdpary_api_settings);
    ...
    public function api_settings_data(Request $request) // works for both email+sms and autoresponder
    {
        ...
        $table = $is_thirdparty_api=='1' ? "settings_thirdparty_apis" : $table;
        ...
        $icon = $is_autoresponder=='0' ? '<i class="fa fa-edit pt-1"></i>' : '<i class="fa fa-sync pt-1"></i>';
        foreach ($info as $key => $value)
        {
            $value->updated_at = date("jS M y H:i:s",strtotime($value->updated_at));
            $str="";
            if(config('app.is_demo')!='1' && has_module_action_access($this->module_id_settings,2,$this->team_access,$this->is_manager))
            $str=$str."<a class='btn btn-circle btn-outline-warning ".$edit_class."' data-id='".$value->id."' href='#'>".$icon."</a>";
            if(has_module_action_access($this->module_id_settings,3,$this->team_access,$this->is_manager))
            $str=$str."&nbsp;<a href='".$delete_route."' data-id='".$value->id."' data-table-name='".$datatable_name."' class='delete-row btn btn-circle btn-outline-danger'>".'<i class="fa fa-trash pt-1"></i>'."</a>";
            $value->actions = "<div class='w-min-50px'>".$str."</div>";
        }
    ...
    public function save_api_settings(Request $request,AutoResponderServiceInterface $autoresponder_service) // works for both email/sms and autoresponder (insert+update)
    {
        ...
        if(!$this->is_admin || !has_module_action_access($this->module_id_settings,[1,2],$this->team_access,$this->is_manager)) {
        ...
        $insert_data['system_prompt'] = $system_prompt;
        $insert_data['chat_model'] = $chat_model;
        $insert_data['settings_data'] = json_encode($form_data);
        $insert_data['updated_at'] = date("Y-m-d H:i:s");

        $table = 'settings_sms_emails';
        if($is_autoresponder=='0'){
            if($is_thirdparty_api=='1') {
                $insert_data['api_type'] = 'ai';
                $table = 'settings_thirdparty_apis';
            }
            else if($is_sms=='1') $insert_data['api_type'] = 'sms';
    ...
    {
        if(config('app.is_demo')=='1' || !$this->is_admin || !has_module_action_access($this->module_id_settings,[1,2],$this->team_access,$this->is_manager)) abort('403');
        $id = $request->id;
        $is_autoresponder = $request->is_autoresponder ?? '0';
        $is_thirdparty_api = $request->is_thirdparty_api ?? '0';
        $table = $is_autoresponder=='0' ? 'settings_sms_emails' : 'settings_email_autoresponders';
        ...
        $response->system_prompt = $email_settings_data->system_prompt ?? '';
    ...
    public function delete_api_settings_action(Request $request){
        ...
        if(!$this->is_admin || !has_module_action_access($this->module_id_settings,3,$this->team_access,$this->is_manager)) {
            return response()->json(['error' => true,'message' => __('You are not allowed to perform this action.')]);
        }
        $id = $request->id;
    ...
    public function delete_thirdparty_api_settings_action(Request $request){
        if(config('app.is_demo')=='1')
        return response()->json(['error' => true,'message' => __('This feature has been disabled in this demo version. We recommend to sign up as user and check.')]);
    ...
    public function manual_transaction_log_data(Request $request)
    {
        ...
        foreach ($info as $key => $value)
        {
            ...
            if (!$this->is_member) $value->name = '<a class="text-decoration-none" href="' . route('update-user',$value->buyer_user_id) . '" target="_blank">' . $value->name . '</a>';
    ...
    {
        if (! request()->ajax() || ('Admin' != $this->user_type)) {
            $message = __('Bad Request.');
            return Response::json(['msg' => $message]);
            exit;
        }
    ...
    private function manual_payment_display_attachment($file)
    {
        ...
        $output .= '<div class="mp-img-item btn btn-circle btn-outline-info" data-image="' . $file . '" href="' . $file . '">';
        $output .= '<i class="fa fa-image"></i>';
        $output .= '</div>';
        $output .= '</div>';
        // dynamic js
        $output .= '<script>"use strict";$(".mp-display-img").Chocolat({className: "mp-display-img", imageSelector: ".mp-img-item"});</script>';
    ...
    public function ai_chat_settings()
    {
        if(!$this->is_admin) abort('403');
        $data['body'] = 'member/settings/ai-chat-settings';
        $data['load_datatable'] = true;
        $data['language_list'] =$this->get_available_language_list();
    ...
    public function ai_chat_settings_action(Request $request)
    {
        if(config('app.is_demo')=='1')
        return response()->json(['error' => true,'message' => __('This feature has been disabled in this demo version. We recommend to sign up as user and check.')]);
        ...
        $api_name = $request->api_name ?? 'openai';
        $chat_model = $request->chat_model ?? '';
        $custom_prompt = $request->custom_prompt ?? '';
        $custom_img = $request->file('logo');

        $rules['custom_img'] = 'nullable|sometimes|image|mimes:png,jpg,jpeg,webp|max:1024';
        $request->validate($rules);

        if ($custom_img) {

            $file = $custom_img;
            $extension = $custom_img->getClientOriginalExtension();
            $filename =  $this->user_id.time().'.'.$extension;
            $upload_dir_subpath = 'public/template/prompt/';

            if(env('AWS_UPLOAD_ENABLED')){
               try {
                   $upload2S3 = Storage::disk('s3')->putFileAs('agency', $file,$filename);
                   $insert_data['custom_prompt_img'] = Storage::disk('s3')->url($upload2S3);
               }
               catch (\Exception $e){
                   $error_message = $e->getMessage();
               }
                ...
                $insert_data['custom_prompt_img'] = asset('storage/app/public/template/prompt').'/'.$filename;
            }
        }

        $insert_data['api_name'] = $api_name;
        $insert_data['profile_name'] = $profile_name;
        $insert_data['custom_prompt'] = $custom_prompt;
        $insert_data['chat_model'] = $chat_model;
        $insert_data['updated_at'] = $now;

        DB::table('ai_chat_settings')->updateOrInsert(['id'=>$request->update_id],$insert_data);
        return response()->json(['error'=>false]);
    }

    public function ai_chat_settings_data(Request $request) 
    {
        $search_value = !is_null($request->input('search.value')) ? $request->input('search.value') : '';
        $is_thirdparty_api = $request->is_thirdparty_api ?? '0';
        $display_columns = array("#","CHECKBOX",'profile_name','custom_prompt','updated_at','actions');
        $search_columns = array('profile_name');
        ...
        $edit_class = 'update-ai-settings-row';
        $delete_route =  route('delete-ai-chat-settings-action');
    ...
    public function delete_ai_chat_settings_action(Request $request){
        if(config('app.is_demo')=='1')
        return response()->json(['error' => true,'message' => __('This feature has been disabled in this demo version. We recommend to sign up as user and check.')]);
    ...
    public function edit_ai_chat_settings_action(Request $request){
        $id = $request->id;
        $ai_chat_settings_data = DB::table('ai_chat_settings')->where('id',$id)->first();
        $response = [];
        $response['profile_name'] = $ai_chat_settings_data->profile_name ?? '';
        $response['custom_prompt'] = $ai_chat_settings_data->custom_prompt ?? '';
        $response['chat_model'] = $ai_chat_settings_data->chat_model ?? '';
        $response['profile_img'] = !empty($ai_chat_settings_data->custom_prompt_img) ?  $ai_chat_settings_data->custom_prompt_img : '';

        return json_encode($response);
    }


    public function ai_chat_profile_dropdown(Request $request){
        ...
        $info_type = DB::table('ai_chat_settings')->orderByRaw('api_name ASC')->get();
