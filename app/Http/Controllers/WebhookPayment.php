...
class WebhookPayment extends Home
{
    ...
    {
        $response_raw = get_domain_only(env('APP_URL'))=='aipen.test' ? file_get_contents('https://ezsoci.com/botsailor-test-ipn/paypal-response.txt') : $request->response_raw;;
        $payment_info = json_decode($response_raw,TRUE);
        $post_api_from_ipn = $payment_info['api_key'] ?? "";
        if($post_api_from_ipn != $this->api_key && get_domain_only(env('APP_URL'))!='aipen.test') exit();
        ...
        //https://developer.paypal.com/api/nvp-soap/ipn/IPNandPDTVariables/#ipn-transaction-types
    ...
    public function stripe_ipn($buyer_user_id=null,$parent_user_id=null,$package_id=null)
    {
        ...
        if($this->complete_payment($insert_data,$payment_type)) session(['payment_success' => '1']);
    ...
    public function mercadopago_action($package_id=0,$buyer_user_id=0,$parent_user_id=0)
    {
        ...
        if(get_domain_only(env("APP_URL")) == "aipen.test") {
