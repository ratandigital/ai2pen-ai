...
$(document).ready(function() {
	...
	$('.myTab .nav-link').on('shown.bs.tab', function (e) {
		...
		if(link_id!='email-tab' && link_id!='sms-tab' && link_id!='emailauto-tab'&& link_id!='thirdparty-api-tab') return false;
        ...
        else if(link_id=='thirdparty-api-tab'){
            table_name = 'mytable4';
            is_thirdparty_api = 1;
        }
	...
	$(document).on('click','.update-api-settings-row',function(e){
		...
		$.ajax({
            ...
            {
                $(modal_id).modal('show');
                $('#system_prompt').val(response.system_prompt);
                setTimeout(function(){
...
function reload_default_thirdparty_api() {

    $.ajax({
        url: common_function_url_get_thirdparty_api_profile_dropdown,
        method: "POST",
        data: {icon:true},
        beforeSend: function (xhr) {
            xhr.setRequestHeader('X-CSRF-TOKEN', csrf_token);
